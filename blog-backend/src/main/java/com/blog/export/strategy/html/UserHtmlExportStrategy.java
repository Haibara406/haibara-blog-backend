 ...
 * 
 * @author haibara
 * @description 将用户数据导出为HTML格式
 * @since 2025/1/21
 */
@Slf4j
@Component
public class UserHtmlExportStrategy implements ExportStrategy<UserListVO> {
    
    @Resource
    private TemplateEngine templateEngine;
    
    private static final String TEMPLATE_NAME = "export/user-export";
    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
    
    @Override
    public ExportResult export(List<UserListVO> data, String fileName) {
        try {
            log.info("开始导出用户数据为HTML格式，数据条数: {}, 文件名: {}", data.size(), fileName);
            
            // 准备模板数据
            Context context = new Context();
            context.setVariable("userList", data);
            ...
            log.info("用户HTML导出完成，文件大小: {} bytes", contentBytes.length);
