 ...
 * 
 * @author haibara
 * @description 将标签数据导出为HTML格式
 * @since 2025/1/21
 */
@Slf4j
@Component
public class TagHtmlExportStrategy implements ExportStrategy<TagVO> {
    
    @Resource
    private TemplateEngine templateEngine;
    
    private static final String TEMPLATE_NAME = "export/tag-export";
    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
    
    @Override
    public ExportResult export(List<TagVO> data, String fileName) {
        try {
            log.info("开始导出标签数据为HTML格式，数据条数: {}, 文件名: {}", data.size(), fileName);
            
            // 准备模板数据
            Context context = new Context();
            context.setVariable("tagList", data);
            ...
            log.info("标签HTML导出完成，文件大小: {} bytes", contentBytes.length);
